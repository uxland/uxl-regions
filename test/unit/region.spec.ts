...
describe("Given an instance of Region", () => {
  ...
  beforeEach(() => {
    ...
    jest.resetAllMocks();
    validateViewStub = jest
      .spyOn(validateView, "validateView")
  ...
  describe("and a view is added", () => {
    ...
    it("should raise error if validate view raises", async () => {
      ...
      let view = {};
      let region = regionFactory();
      expect(region.addView(mockViewName, {})).rejects.toThrow();
    ...
    it("should raise error if a view with same key is already added", async () => {
      ...
      when(<any>jest.spyOn(region, "getView"))
        ...
        .mockReturnValue({});
      await expect(region.addView(mockViewName, {})).rejects.toThrow(
  ...
  describe("and a view is activated", () => {
    describe("by key", () => {
      ...
      it("should be added only once", async () => {
        ...
        await region.activate(mockViewName);
        await region.activate(mockViewName);
        expect(region.currentActiveViews.filter((v) => v === view).length).toBe(
          1
        );
      });
    ...
    describe("by view", () => {
      ...
      it("should raise error if region does not contain view", async () => {
        let region = regionFactory();
        await expect(region.activate({})).rejects.toThrow(
  ...
  describe("isAtiveView suite", () => {
    it("should raise exception if view does not exists", () => {
      ...
      expect(() => region.isViewActive("my-view2")).toThrow(
        `region ${mockReginnName} doest not contain this view`
      );
      expect(() => region.isViewActive({})).toThrow(
        `region ${mockReginnName} doest not contain this view`
      );
      jest.resetAllMocks();
      jest.restoreAllMocks();
