...
export interface IRegionManager {
    ...
    registerViewWithRegion(regionName: string, key: string, view: ViewDefinition): IRegionManager;
...
export class RegionManager implements IRegionManager {
    ...
    remove(region: string | IRegion): IRegion {
        ...
        if (regionName && result)
            viewRegistry.unregisterRegion(regionName, result);
        return result;
    }
